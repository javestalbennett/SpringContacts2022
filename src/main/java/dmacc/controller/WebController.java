...
public class WebController {
	...
	public String addNewContact(@ModelAttribute ListItem c, Model model) {
		repo.save(c);
	...
	public String showUpdateContact(@PathVariable("id") long id, Model model) {
		ListItem c = repo.findById(id).orElse(null);
		model.addAttribute("newContact", c);
		return "input";
	}

	@PostMapping("/update/{id}")
	public String reviseContact(ListItem c, Model model) {
		repo.save(c);
		return viewAllContacts(model);
	}
